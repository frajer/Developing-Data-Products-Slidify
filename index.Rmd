---
title       : Predicting Miles Per Gallon
subtitle    : 
author      : frajer
output      : ioslides_presentation
mode        : self-contained
--- 

<style>

.title-slide hgroup > h1,
.title-slide hgroup > h2 {
    color:  #00BFFF;
}

slide.segue h2{
    color: #00BFFF;
}
slide:not(.segue) h2{
    color: #00BFFF;
}
</style>

## ShinyApps purpose
<br>

> 1. Showing tech specs for specifed car<br>
> 2. Showing actual MPG value for specifed car<br>
> 3. Showing predicted MPG value for specifed car


## Predicting formula
<br>

> * The model for predicting the MPG value is based on my work for "Regression Models" course project.<br>
> * The formula for the model is: 
<div style='text-align: center;'>
    <br>
    <img src='model.png' />
</div>


## App screenshoot

<img src='app.png' width="800"/>



## Evaluating R expression for predicted value

```{r}
techs <- function(car) mtcars[car,]
car <- "Mazda RX4"
round(9.618-3.917*techs(car)$wt+1.226*techs(car)$qsec
      +2.936*techs(car)$am,digits=2)
```

